...
function createFormAutoGenerate(user, items, statusFlag, excludeFormId = null){
  ...
  const stmt = db.prepare("INSERT INTO reimbursements (user_id, amount, purpose, type, remark, invoice_number, invoice_date, buyer_name, service_name, invoice_amount, status, created_at, form_status, form_id) VALUES (?,?,?,?,?,?,?,?,?,?,'已归集到报销单',datetime('now', 'localtime'),'已绑定',?)");
...
function updateForm(formId, user, items, statusFlag){
  ...
  const tx = db.transaction(()=>{
    ...
    const keep=new Set();
  const insertStmt = db.prepare("INSERT INTO reimbursements (user_id, amount, purpose, type, remark, invoice_number, invoice_date, buyer_name, service_name, invoice_amount, status, created_at, form_status, form_id) VALUES (?,?,?,?,?,?,?,?,?,?,'已归集到报销单',CURRENT_TIMESTAMP,'已绑定',?)");
    const updateStmt = db.prepare('UPDATE reimbursements SET amount=?, purpose=?, type=?, remark=?, invoice_number=?, invoice_date=?, buyer_name=?, service_name=?, invoice_amount=? WHERE id=? AND form_id=?');
