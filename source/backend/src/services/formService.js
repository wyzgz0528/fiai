...
function generateFormNumber(){
  ...
  const seq=(row.c||0)+1;
  return `RB${today}${String(seq).padStart(4,'0')}`;
}
...
function createFormAutoGenerate(user, items, statusFlag, excludeFormId = null){
  ...
  // 🔧 修复：报销记录状态应与报销单状态保持一致
  const recordStatus = statusFlag === '提交申请' ? '待财务审核' : '草稿';
  const recordFormStatus = statusFlag === '提交申请' ? '已提交' : '已绑定';
  // 🔧 修复：approval_status也应该与报销单状态保持一致
  const recordApprovalStatus = statusFlag === '提交申请' ? 'pending' : '草稿';
  console.log(`🔍 [DEBUG] createFormAutoGenerate - 报销单状态: ${status}, 报销记录状态: ${recordStatus}, 审批状态: ${recordApprovalStatus}, 表单状态: ${recordFormStatus}`);
  const stmt = db.prepare("INSERT INTO reimbursements (user_id, amount, purpose, type, remark, invoice_number, invoice_date, buyer_name, service_name, invoice_amount, status, created_at, form_status, form_id, approval_status) VALUES (?,?,?,?,?,?,?,?,?,?,?,datetime('now', 'localtime'),?,?,?)");
...
function updateForm(formId, user, items, statusFlag){
  ...
  // 🔍 调试：检查updateForm接收到的数据
  console.log('🔍 [DEBUG] updateForm接收数据:', {
    formId,
    statusFlag,
    itemsCount: items.length,
    sampleOriginalItem: items[0] ? {
      invoice_number: items[0].invoice_number,
      invoice_date: items[0].invoice_date,
      buyer_name: items[0].buyer_name,
      service_name: items[0].service_name
    } : null,
    sampleProcessedItem: processedItems[0] ? {
      invoice_number: processedItems[0].invoice_number,
      invoice_date: processedItems[0].invoice_date,
      buyer_name: processedItems[0].buyer_name,
      service_name: processedItems[0].service_name
    } : null
  });
  ...
  const tx = db.transaction(()=>{
    ...
    const keep=new Set();
  // 🔧 修复：报销记录状态应与报销单状态保持一致
  const currentForm = db.prepare('SELECT status FROM reimbursement_forms WHERE id=?').get(formId);
  const recordStatus = currentForm.status === '草稿' ? '草稿' : '待财务审核';
  const recordFormStatus = currentForm.status === '草稿' ? '已绑定' : '已提交';
  // 🔧 修复：approval_status也应该与报销单状态保持一致
  const recordApprovalStatus = currentForm.status === '草稿' ? '草稿' : 'pending';
  console.log(`🔍 [DEBUG] updateForm - 报销单状态: ${currentForm.status}, 报销记录状态: ${recordStatus}, 审批状态: ${recordApprovalStatus}, 表单状态: ${recordFormStatus}`);
  const insertStmt = db.prepare("INSERT INTO reimbursements (user_id, amount, purpose, type, remark, invoice_number, invoice_date, buyer_name, service_name, invoice_amount, status, created_at, form_status, form_id, approval_status) VALUES (?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,?,?,?)");
    // 🔧 修复：保护OCR已保存的发票字段，只在有值时更新
    ...
    for(const it of processedItems){
      ...
      if(it.id){
        ...
        console.log('🔍 [DEBUG] UPDATE参数:', {
          ...
          buyer_name: it.buyer_name || null,
          service_name: it.service_name || null,
          invoice_amount: it.invoice_amount || null,
          formId
        });
        ...
        updateStmt.run(
          ...
          it.invoice_date || null,
          it.buyer_name || null,
          it.service_name || null,
          it.invoice_amount || null,
          it.id,
          formId
        );
...
function submitForm(formId, user){
  ...
  // 🔧 修复：无论从什么状态提交，都要更新报销记录状态
  try {
    try { ensureApprovalSchema(); } catch(_) {}

    if (['rejected','finance_rejected','manager_rejected'].includes(norm)) {
      // 从已驳回状态重新提交，重置所有明细的审批状态为待审核，清除上一轮审批字段
      db.prepare("UPDATE reimbursements SET approval_status = 'pending', approver_id = NULL, approved_at = NULL, reject_reason = NULL WHERE form_id = ?")
        .run(formId);
      // 恢复记录状态到通用值，避免前端从旧字段读取出“已驳回”；同步恢复 form_status
      try {
      ...
      // 🔧 修复：从草稿状态提交，更新报销记录状态为pending
      console.log(`🔍 [DEBUG] submitForm - 从草稿状态提交，更新报销记录状态`);
      db.prepare("UPDATE reimbursements SET approval_status = 'pending', status = '已归集到报销单', form_status = '已提交' WHERE form_id = ?")
        .run(formId);
    }
  } catch(error) {
    console.warn('更新报销记录状态失败:', error);
  }
...
function withdrawForm(formId, user){
  console.log('🔍 [DEBUG] withdrawForm 开始撤回报销单:', formId);
  ...
  try {
    ...
    console.log('🔍 [DEBUG] 撤回后保留发票字段，只重置审批状态');
  ...
  console.log('🔍 [DEBUG] withdrawForm 撤回完成，发票字段已保留');
