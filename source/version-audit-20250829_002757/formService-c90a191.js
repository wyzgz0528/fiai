...
function createFormAutoGenerate(user, items, statusFlag, excludeFormId = null){
  console.log('🔍 [DEBUG] createFormAutoGenerate 接收到的原始数据:', JSON.stringify(items, null, 2));
  ...
  const processedItems = items.map(item => ({
    ...
    invoice_amount: item.invoice_amount ? Number(item.invoice_amount) : null,
    invoice_date: item.invoice_date || null,
    buyer_name: item.buyer_name || null,
    service_name: item.service_name || null
  }));

  console.log('🔍 [DEBUG] 统一处理后的发票数据(5个基本字段):', JSON.stringify(processedItems.map(item => ({
  ...
  const stmt = db.prepare("INSERT INTO reimbursements (user_id, amount, purpose, type, remark, invoice_number, invoice_amount, invoice_date, buyer_name, service_name, status, created_at, form_status, form_id) VALUES (?,?,?,?,?,?,?,?,?,?,'已归集到报销单',datetime('now', 'localtime'),'已绑定',?)");
  const voucherStmt = db.prepare('INSERT INTO vouchers (reimbursement_form_id, original_name, file_name, file_path, file_size, file_type, uploaded_by, created_at) VALUES (?,?,?,?,?,?,?,datetime(\'now\', \'localtime\'))');
    ...
    for(const item of processedItems){
      ...
      const recInfo = stmt.run(user.userId, item.amount, item.purpose, item.type || '', item.remark || '', item.invoice_number || null, item.invoice_amount || null, item.invoice_date || null, item.buyer_name || null, item.service_name || null, formId);
...
function updateForm(formId, user, items, statusFlag){
  console.log('🔍 [DEBUG] updateForm 接收到的原始数据:', JSON.stringify(items, null, 2));
  ...
  console.log('🔍 [DEBUG] updateForm 统一处理后的发票数据(5个基本字段):', JSON.stringify(processedItems.map(item => ({
    id: item.id,
    invoice_number: item.invoice_number,
    invoice_amount: item.invoice_amount,
    invoice_date: item.invoice_date,
  ...
  const tx = db.transaction(()=>{
    ...
    const keep=new Set();
  const insertStmt = db.prepare("INSERT INTO reimbursements (user_id, amount, purpose, type, remark, invoice_number, invoice_amount, invoice_date, buyer_name, service_name, status, created_at, form_status, form_id) VALUES (?,?,?,?,?,?,?,?,?,?,'已归集到报销单',CURRENT_TIMESTAMP,'已绑定',?)");
    const updateStmt = db.prepare('UPDATE reimbursements SET amount=?, purpose=?, type=?, remark=?, invoice_number=?, invoice_amount=?, invoice_date=?, buyer_name=?, service_name=? WHERE id=? AND form_id=?');
  const voucherStmt = db.prepare('INSERT INTO vouchers (reimbursement_form_id, original_name, file_name, file_path, file_size, file_type, uploaded_by, created_at) VALUES (?,?,?,?,?,?,?,CURRENT_TIMESTAMP)');
    ...
    for(const it of processedItems){
      ...
      if(it.id){
        updateStmt.run(amount, it.purpose, it.type, it.remark || '', it.invoice_number || null, it.invoice_amount || null, it.invoice_date || null, it.buyer_name || null, it.service_name || null, it.id, formId);
        ...
        const info = insertStmt.run(user.userId, amount, it.purpose, it.type, it.remark || '', it.invoice_number || null, it.invoice_amount || null, it.invoice_date || null, it.buyer_name || null, it.service_name || null, formId);
